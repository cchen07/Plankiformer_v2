...
def plot_performance_overview(model_name, test_dataset, accuracy, f1_score, BC, outpath, remove_0):
...
def read_extra_test_report(test_extra_report_file):
    ...
    NAE_value = format(float(extra_test_report.iloc[12].item()), '.5f')
    ...
    return bias_value, BC_value, MAE_value, MSE_value, RMSE_value, R2_value, NAE_value, AE_rm_junk_value, NAE_rm_junk_value
...
def performance_matrix(model_performance_paths, testsets, finetuned, ensemble, remove_0, TTA):
    ...
    NAE = np.zeros([n_model, n_dataset])
    ...
    for i, imodel_path in enumerate(model_performance_paths):
        ...
        for j, idataset in enumerate(dataset_names):
            ...
            bias_value, BC_value, MAE_value, MSE_value, RMSE_value, R2_value, NAE_value, AE_rm_junk_value, NAE_rm_junk_value = read_extra_test_report(extra_test_report_file)
            accuracy[i, j], f1_score[i, j] = accuracy_value, f1_value
            bias[i, j], BC[i, j], MAE[i, j], MSE[i, j], RMSE[i, j], R2[i, j], NAE[i, j], AE_rm_junk[i, j], NAE_rm_junk[i, j] = bias_value, BC_value, MAE_value, MSE_value, RMSE_value, R2_value, NAE_value, AE_rm_junk_value, NAE_rm_junk_value
    
    return accuracy, f1_score, bias, BC, MAE, MSE, RMSE, R2, NAE, AE_rm_junk, NAE_rm_junk, test_dataset


def plot_performance_curve(model_name, test_dataset, accuracy, f1_score, BC, outpath, remove_0):
...
def plot_performance(aug_types, test_dataset, accuracy, f1_score, BC, outpath, remove_0):
    n_aug = len(aug_types)
    ...
    for i, aug_type in enumerate(aug_types):
        ...
        plt.errorbar(x=np.arange(len(test_dataset)), y=mean_BC, yerr=sem_BC, fmt='s', capsize=5, label=aug_type)
    plt.legend()
...
if __name__ == '__main__':
    accuracy, f1_score, bias, BC, MAE, MSE, RMSE, R2, NAE, AE_rm_junk, NAE_rm_junk, test_dataset = performance_matrix(args.model_performance_paths, args.testsets, args.finetuned, args.ensemble, args.remove_0, args.TTA)
    model_name = args.model_names
    outpath = args.outpath
    aug_types = args.aug_types
    plot_performance_overview(model_name, test_dataset, accuracy, f1_score, BC, outpath, args.remove_0)
    plot_performance_curve(model_name, test_dataset, accuracy, f1_score, BC, outpath, args.remove_0)
    plot_performance(aug_types, test_dataset, accuracy, f1_score, BC, outpath, args.remove_0)

    plot_performance_AE(aug_types, test_dataset, NAE, AE_rm_junk, NAE_rm_junk, outpath, args.remove_0)
