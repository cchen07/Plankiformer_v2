...
class import_and_train_model:
    ...
    def import_deit_models(self, train_main, data_loader):
        ...
        if train_main.params.add_layer == 'yes':
            if train_main.params.architecture == 'deit':
                ...
                additional_layers = nn.Sequential(
                                        nn.Dropout(p=train_main.params.dropout_1),
                                        nn.Linear(in_features=in_features, out_features=train_main.params.fc_node),
                                        nn.ReLU(inplace=True),
                                        nn.Dropout(p=train_main.params.dropout_2),
                                        nn.Linear(in_features=train_main.params.fc_node, out_features=num_classes),
                                        )
        ...
        elif train_main.params.add_layer == 'no':
            if train_main.params.last_layer_finetune == 'yes':
                n_layer = 0
                for param in self.model.parameters():
                    n_layer += 1
    ...
    def run_training(self, train_main, data_loader, initial_epoch, epochs, lr, name, best_values, modeltype):
        ...
        if train_main.params.valid_set == 'yes':
            for epoch in range(initial_epoch, epochs):
                ...
                plt.xscale('log')
                plt.legend()
                plt.savefig(data_loader.checkpoint_path + '/updated_performance_curves_logx_' + name + '.png')
                plt.close()

                # # rescaled axis
                # step = list(range(1, len(train_accuracies) + 1))
                # if name == 'original':
                #     rescaled_step = (np.array(step) * train_main.params.batch_size / train_main.params.lr)
                # elif name == 'tuned' or name == 'finetuned':
                #     rescaled_step = (np.array(step) * train_main.params.batch_size / train_main.params.finetune_lr)
                # plt.figure(figsize=(15, 4))
                # plt.subplot(1, 3, 1)
                # plt.plot(rescaled_step, train_accuracies, label='Train accuracy')
                # plt.plot(rescaled_step, test_accuracies, label='Validation accuracy')
                # plt.xscale('log')
                # plt.legend()
                # plt.subplot(1, 3, 2)
                # plt.plot(rescaled_step, train_f1s, label='Train F1')
                # plt.plot(rescaled_step, test_f1s, label='Validation F1')
                # plt.xscale('log')
                # plt.legend()
                # plt.subplot(1, 3, 3)
                # plt.plot(rescaled_step, train_losses, label='Train loss')
                # plt.plot(rescaled_step, test_losses, label='Validation loss')
                # plt.xscale('log')
                # plt.legend()
            ...
            for epoch in range(initial_epoch, epochs):
                ...
                if train_main.params.save_best_model_on_loss_or_f1_or_accuracy == 1:
                    ...
                    if train_accuracy > best_acc1 or epoch == 1:
                        torch.save({'model_state_dict': self.model.state_dict(),
                                    'optimizer_state_dict': self.optimizer.state_dict(),
                                    'loss': train_loss,
                                    'f1': train_f1,
                ...
                best_acc1 = max(train_accuracy, best_acc1)
                best_f1 = max(train_f1, best_f1)
                best_loss = min(train_loss, best_loss)

                total_mins_per_epoch = (time() - time_begin_epoch) / 60

                print('[Train] Acc:{}, F1:{}, loss:{}, epoch time (in mins) :{}'.format(
                    np.round(train_accuracy, 3),
                    np.round(train_f1, 3),
                    np.round(train_loss, 3),
                    np.round(total_mins_per_epoch, 3)))
        ...
        # Logs = [train_losses, train_accuracies, train_f1s]
    ...
    def run_prediction_on_unseen_with_y(self, train_main, test_main, data_loader, name):
        ...
        if len(test_main.params.model_path) > 1:
            ...
            first_indices = prob.argsort()[:, -1]
            confs = [prob[i][first_indices[i]] for i in range(len(first_indices))]
            np.savetxt(test_main.params.test_outpath + '/Prediction_confidence.csv', confs)
            for i in range(len(confs)):
                if confs[i] < test_main.params.threshold:
                    output_corrected_label[i] = 'unknown'
            ...
            # plt.legend()
            plt.tight_layout()
            plt.savefig(test_main.params.test_outpath + 'Population_Symmetric_Absolute_Percentage_Error.png', dpi=300)
            plt.close()
            ...
            # plt.legend()
            plt.tight_layout()
            plt.savefig(test_main.params.test_outpath + 'Population_Normalized_Absolute_Error.png', dpi=300)
            plt.close()
            ...
            # plt.legend()
            plt.tight_layout()
            plt.savefig(test_main.params.test_outpath + 'Population_Absolute_Percentage_Error.png', dpi=300)
            plt.close()
...
def cls_train(train_main, train_loader, model, criterion, optimizer, clip_grad_norm, modeltype):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if train_main.params.add_layer == 'yes':
            if train_main.params.architecture == 'deit' or train_main.params.architecture == 'vit' or train_main.params.architecture == 'mae':
